...
public class BuyingACar {
    public static int[] nbMonths(int startPriceOld, int startPriceNew, int savingperMonth, double percentLossByMonth) {
        ...
        double allMoney = oldCar;
        double percent = percentLossByMonth / 100;

        while (allMoney < newCar) {
            months++;
            oldCar -= oldCar * percent;
            newCar -= newCar * percent;
            percentLossByMonth = months % 2 != 0 ? percentLossByMonth + 0.5 : percentLossByMonth;
            percent = percentLossByMonth / 100;
            allMoney = months * savingperMonth + oldCar;
        }
        int money = (int)Math.round(allMoney - newCar);
        return new int[]{months, money};
    }
